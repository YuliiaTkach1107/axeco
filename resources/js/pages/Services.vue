...
useHead({
  title: 'Nos Services - Axeco | Gestion de Copropriété à 360°',
  meta: [
    { name: 'description', content: 'Découvrez les services d’Axeco : gestion technique, administrative et financière pour votre copropriété. Une expertise transparente et rigoureuse.' },
    { property: 'og:title', content: 'Nos Services - Axeco' },
    { property: 'og:description', content: 'Solutions professionnelles pour la gestion de copropriété.' },
...
<MainLayout>
  <!-- Hero Section -->
    <section class="hero relative min-h-[700px] h-screen w-full overflow-hidden flex items-center" style="background: linear-gradient(180deg, rgba(232,243,255,0.8) 0%, rgba(255,255,255,0.5) 100%)">
        <div class="hidden md:flex absolute left-15 top-50 w-[150px] h-[150px] border-3 border-[#F2522E] rotate-22 rounded-[33px] opacity-20"></div>
        <div class="hidden md:flex absolute left-170 top-140 w-[100px] h-[100px] border-3 border-[#2E7EED] -rotate-54 rounded-[33px] opacity-20"></div>
        <div class="px-6 flex md:flex-col justify-center lg:flex-row relative z-20 w-full max-w-7xl mx-auto pt-25 lg:pt-40 h-[100%]">
    ...
    <p class="text-[16px] lg:text-[18px] text-[#627a9a] max-w-[90%] lg:max-w-[50%] leading-relaxed mt-1 mb-6">Des solutions complètes et personnalisées pour la gestion de votre copropriété</p>
    ...
    <div class="relative hidden lg:flex w-[45vw] max-w-[600px] mr-20 items-center h-[100%]">

    <img
        src="/images/services/hero-analysis.jpg"
        class="absolute top-[300px] left-[140px] -rotate-7 z-10
        rounded-[20px] border-6 border-[#B9D6FE]
        w-[35%] h-[25%] object-cover"
    />

    <img
        src="/images/services/hero-building.png"
        class="absolute top-[66%] left-[70%] rotate-5 z-10
        rounded-[20px] border-6 border-[#B9D6FE]
        w-[40%] h-[30%] object-cover"
    />

    <img
        src="/images/services/hero-office.png"
        class="absolute z-0 left-[250px] top-10
        w-[70%] h-[85%]
        rounded-[20px] border-6 border-[#B9D6FE]
        rotate-12 object-cover"
    />
</div>
        </div>
    </section>
...
<section class="services py-8 lg:py-20" style="background-image: linear-gradient(to bottom, #FFFFFF 0%, #F0F6FC 50%, #FFFFFF 100%);">
  <div class="max-w-7xl mx-auto px-6">

    <div 
      v-for="(service, index) in services" 
      :key="index"
      class="mb-18 lg:mb-28"
    >
      <div 
        :id="service.anchor"
        :class="[
      ...
      >
        <span class="text-sm uppercase tracking-widest font-semibold text-[#0D4677]/60" :style="{ color: service.accentColor}">
          Service {{ index + 1 }}
        </span>
      </div>
  ...
  <div class="w-full lg:w-1/2">
    <div class="bg-white rounded-[28px] shadow-xl overflow-hidden">
      ...
      <div class="p-6 md:p-8">
        <h2 class="text-[26px] md:text-[30px] font-semibold text-[#0D4677]">
          {{ service.title }}
        </h2>
      </div>
    </div>
  ...
  <!-- Описание и раскрывающийся список -->
  <div class="w-full lg:w-1/2 flex flex-col justify-start">
    <p 
      class="text-[#627a9a] text-[16px] lg:text-[18px] leading-relaxed mb-4"
    ...
    <button
      ...
      class="group cursor-pointer text-[#1654d8] text-[20px] font-normal flex items-center gap-2 mb-2"
    >
      <span class='text-[18px]'>{{ openDetailsIndex === index ? '−' : '+' }}</span> 
      Détails
    </button>
  ...
  <ul 
    ...
    class="list-disc ml-6 text-[#627a9a] space-y-1 text-[16px]"
  >
...
<!-- Desktop -->
   ...
   <div class="grid gap-10 grid-cols-1 lg:grid-cols-4 lg:max-w-[95%] ">
      <div
         ...
         >
         <div
            class="relative h-[100%] rounded-[24px] p-5
            border-2 border-[#c6e0fa] bg-white
            ...
            <!-- Icon -->
            <div
               class="w-[60px] h-[60px] rounded-full mb-8 mx-auto
               flex items-center justify-center bg-[rgb(143,155,191,0.3)]"
               v-html="valeur.icon"
               />
               <!-- Title -->
    ...
    >
      <div
        class="flex flex-col w-full
               min-h-[260px] 
               rounded-[20px] p-6
               border border-[#c6e0fa]
               bg-white"
      >

        <!-- Icon -->
        <div
          class="w-[50px] h-[50px] rounded-full mb-5 mx-auto
                 flex items-center justify-center
                 bg-[rgb(143,155,191,0.2)]"
          v-html="valeur.icon"
        ></div>

        <!-- Title -->
        <h3 class="text-[18px] font-semibold text-[#0d4677] text-center mb-3">
          {{ valeur.title }}
        </h3>

        <!-- Description -->
        <p class="text-[15px] text-[#627a9a] text-center leading-relaxed flex-grow">
          {{ valeur.text }}
        </p>
