...
<template>
   <button @click="openSettings"  title="Gérer les préférences en matière de cookies" aria-label="Ouvrir les paramètres des cookies" class="fixed bottom-4 left-4 md:bottom-6 md:left-6 bg-[#0d4677] text-white px-2 py-2 rounded-full shadow-lg z-[9999] pointer-events-auto cursor-pointer">
   ...
   <div v-if="showBanner" class="fixed inset-x-4 bottom-20 md:inset-x-auto md:bottom-8 md:right-8 md:w-[32rem] lg:w-[38rem] bg-[#ffffff] px-5 py-6 md:px-6 md:py-8 border border-[#0d4677] flex flex-col rounded-2xl justify-between items-stretch h-auto shadow-[0_10px_40px_rgba(0,0,0,0.15)] z-50" role="dialog" aria-modal="true" aria-live="polite" aria-label="Bannière de consentement aux cookies">
      <div class="text-sm text-center md:text-left space-y-2">
      ...
      <div class="mt-5 flex flex-col md:flex-row md:flex-wrap justify-center md:justify-start gap-2">
         <button @click="acceptAll" type='button' class="w-full md:w-auto px-4 py-3 bg-background cursor-pointer border border-[#0d4677] rounded hover:cursor-pointer" aria-label="Accepter tous les cookies" >
            Tout accepter
         </button>
         <button @click="rejectAll" type='button' class="w-full md:w-auto bg-[#0d4677] cursor-pointer text-white px-4 py-3 rounded hover:cursor-pointer" aria-label="Refuser tous les cookies">
            Refuser
         </button>
         <button @click="openSettings" type='button' class="w-full md:w-auto underline px-2 py-2 cursor-pointer" aria-label="Personnaliser les préférences en matière de cookies" >
            Personnaliser
         </button>
      </div>
   </div>
   <!-- Fenêtre modale des paramètres -->
   <div v-if="settingsOpen" class="fixed inset-0 bg-black/50 flex items-end md:items-center justify-center z-50 p-3 md:p-4">
      <div class="bg-white w-full max-w-[700px] max-h-[90vh] overflow-y-auto p-5 md:p-8 rounded-2xl shadow-2xl">
         <!-- TITLE -->
         ...
         <div class="flex flex-col-reverse md:flex-row justify-end gap-3 mt-6">
            <button @click="closeSettings" class="w-full md:w-auto px-4 py-2 border rounded cursor-pointer" type='button' > 
                Annuler
            </button>
            <button @click="saveSettings" type='button' class="w-full md:w-auto bg-[#0d4677] text-white px-5 py-2 rounded-lg cursor-pointer" >
                Sauvegarder
            </button>
         </div>
      </div>
